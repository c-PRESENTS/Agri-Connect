      ...
      <main className="mx-auto max-w-5xl px-4 py-6 sm:py-10">
        <div className="mb-6">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Aggregate marketplace health and payment recovery controls.
          </p>
        </div>
        {isLoading ? (
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {[1, 2, 3, 4].map((value) => (
              <div key={value} className="h-28 animate-pulse rounded-xl bg-muted" />
            ))}
          </div>
        ) : isError || !summary ? (
          <div className="py-16 text-center">
            <XCircle className="mx-auto mb-3 h-10 w-10 text-destructive" />
            <p className="font-medium">Unable to load platform summary.</p>
            ...
            <Card className="mt-5">
              <CardContent className="p-4">
                ...
                {Object.keys(summary.statusCounts).length === 0 ? (
                  <p className="mt-3 text-sm text-muted-foreground">No orders have been created yet.</p>
                ) : (
                  <div className="mt-3 flex flex-wrap gap-2">
                    {Object.entries(summary.statusCounts).map(([status, count]) => (
                      <span className="rounded-full bg-muted px-3 py-1 text-sm" key={status}>
                        {status}: {count}
                      </span>
                    ))}
                  </div>
