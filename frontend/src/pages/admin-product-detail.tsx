  ...
  return <AdminLayout><div className="mx-auto max-w-6xl space-y-5">
    ...
    {query.isLoading ? <Card><CardContent className="p-16 text-center"><Loader2 className="mx-auto h-6 w-6 animate-spin" /></CardContent></Card> : query.isError || !query.data ? <Card><CardContent className="space-y-3 p-14 text-center"><PackageSearch className="mx-auto h-9 w-9 text-muted-foreground" /><p className="font-bold">Product detail could not be loaded.</p><Button variant="outline" onClick={() => query.refetch()}>Retry</Button></CardContent></Card> : <>
      ...
      <div className="grid gap-4 lg:grid-cols-[1.4fr_1fr]">
        <Card><CardHeader><CardTitle>Listing preview and gallery</CardTitle></CardHeader><CardContent className="space-y-4"><div className="grid gap-3 sm:grid-cols-2">{(() => { const res = resolveProductImageForProduct({ id: query.data.product.id, name: query.data.product.name, categoryId: query.data.product.categoryId, subcategoryId: query.data.product.subcategoryId, images: query.data.product.images }); return <SafeProductImage src={res.src} fallbackSrc={res.fallbackSrc} alt={query.data.product.name} className="aspect-[4/3] w-full rounded-xl border object-cover" />; })()}</div><p className="whitespace-pre-wrap text-sm">{query.data.product.description}</p><div className="grid gap-3 sm:grid-cols-3"><Info label="Category" value={`${query.data.product.categoryId} / ${query.data.product.subcategoryId}`} /><Info label="Price" value={`${query.data.product.currency} ${query.data.product.price.toFixed(2)} / ${query.data.product.unit}`} /><Info label="Stock" value={`${query.data.product.stock} ${query.data.product.unit}`} /><Info label="Region" value={query.data.product.regionName || "No approved region"} /><Info label="Submitted" value={query.data.product.submittedAt ? new Date(query.data.product.submittedAt).toLocaleString() : "Not submitted"} /><Info label="Reviewed" value={query.data.product.reviewedAt ? new Date(query.data.product.reviewedAt).toLocaleString() : "Not reviewed"} /></div><Button asChild size="sm" variant="outline"><a href={`/products/${encodeURIComponent(query.data.product.id)}`} target="_blank" rel="noreferrer">Open listing route <ExternalLink className="ml-2 h-3.5 w-3.5" /></a></Button></CardContent></Card>
